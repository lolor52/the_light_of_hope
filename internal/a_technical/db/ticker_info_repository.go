...
// GetByID возвращает запись ticker_info по первичному ключу.
...
`
	...
	if errors.Is(err, sql.ErrNoRows) {
		return models.TickerInfo{}, fmt.Errorf("ticker_info %d: %w", id, sql.ErrNoRows)
	}
	if err != nil {
		return models.TickerInfo{}, fmt.Errorf("select ticker_info: %w", err)
	}
