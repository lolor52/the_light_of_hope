...
```
T_open(d)  = начало Основной сессии по расписанию MOEX ISS для BOARDID тикера на дату d (MSK)
T_close(d) = окончание Основной сессии по расписанию MOEX ISS для BOARDID тикера на дату d (MSK)

TA_open_start(d), TA_open_end(d) = интервалы аукциона открытия по расписанию MOEX ISS для тикера/BOARDID на дату d (MSK)
TA_close_start(d), TA_close_end(d) = интервалы аукциона закрытия по расписанию MOEX ISS для тикера/BOARDID на дату d (MSK)
...
```
d_prev = предыдущая дата «Основной» торговой сессии по торговому календарю MOEX для тикера/BOARDID
```
